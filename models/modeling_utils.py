...
def accuracy(logits, labels, ignore_index: int = -100):
    with torch.no_grad():
        ...
        target_array = targets[valid_mask]
        prediction_array = predictions[valid_mask]
        return torch.mean(torch.mul(target_array-prediction_array,target_array-prediction_array))
...
class MLMHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.classifier = SimpleMLP(hidden_size+200, 512, 1, 0.1)

    def forward(self, hidden_states, DNA_data, targets=None, coverage_data=None):
        DNA_data = torch.unsqueeze(DNA_data,1).repeat(1,179,1)
        hidden_states = torch.cat((hidden_states, DNA_data), 2)
        hidden_states = self.classifier(hidden_states)
        hidden_states = torch.squeeze(hidden_states,2)
        outputs = (hidden_states,)
        if targets is not None: 
             
            loss_fct = nn.MSELoss()
            valid_mask = (coverage_data > 40)
            valid_targets = targets[valid_mask]
            valid_prediction = hidden_states[valid_mask]
            masked_lm_loss = loss_fct(valid_prediction, valid_targets)
            ...
            metrics = {'Pearson':
                       torch.mean(acc_fct(valid_prediction, valid_targets))}
...
class ValuePredictionHead(nn.Module):
    ...
    def forward(self, hidden_states, targets=None, coverage_data=None):
        hidden_states = self.classifier(hidden_states)
        ...
        if targets is not None:
            ...
            metrics = {'Pearson':
                       torch.mean(acc_fct(valid_prediction, valid_targets)),
                      'Correlation':
                       Correlation(valid_prediction, valid_targets)}
            loss_and_metrics = (masked_lm_loss, metrics)
            outputs = (loss_and_metrics,) + outputs
        """
        elif targets is not None and coverage_data is not None:
            loss_fct = nn.MSELoss()
            valid_mask = (coverage_data > 10)
            valid_targets = targets[valid_mask]
            valid_prediction = hidden_states[valid_mask]
            masked_lm_loss = loss_fct(valid_prediction, valid_targets)
            ...
            metrics = {'Pearson':
                       torch.mean(acc_fct(valid_prediction, valid_targets)),
                      'Correlation':
                       Correlation(valid_prediction[valid_mask], valid_targets[valid_mask])}
            loss_and_metrics = (masked_lm_loss, metrics)
            outputs = (loss_and_metrics,) + outputs
        """
        return outputs  # (loss), prediction_scores
